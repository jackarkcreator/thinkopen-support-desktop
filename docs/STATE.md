...
- **v1.2.1** (2026-09-19; v1.2.0 = failed mac CI, draft left unpublished) — **Start remote support.** `window.minka.startRemoteSupport()` (`src/remote-support.js`): fetches the ThinkOpen Support RustDesk client from the portal (`/api/remote-sessions/client/<os>`, the user's session), verifies the signature against PINNED signers before EVERY launch (Windows: Authenticode Valid + `O=Purslane`, `SERIALNUMBER=53481265A`; macOS: codesign deep/strict + TeamIdentifier `HZF9JMC8YN` + spctl), caches under `userData/remote-support/` (re-fetch daily), launches it, and returns the hostname (macOS: `scutil --get LocalHostName`, never `os.hostname()`). The web card then finds the RustDesk ID server-side (`auto-id`, mig 108). Intel Macs → `unsupported_os` (the console builds arm64 only). `focusWindow` is now real (notification click). Mac path harness-tested on TKOBEAR incl. a tamper test (modified bundle → `signature_invalid`, cache deleted); the Windows path is proven on the first WK40/VM run. ⚠️ Releases land as DRAFTS: verify + publish by hand.
...
## 2026-09-26 — v1.2.2: macOS updater "takes 2-3 quits to update" fix (port of Okvia staff v1.1.7)
- **Root cause (proven on staff app, `~/Library/Caches/net.thinkopen.minka.ShipIt/ShipIt_stderr.log`, same code here):** every `checkForUpdates()` (8s launch, 6h interval, tray "Check for Updates…") → electron-updater re-downloads (cached) → native Squirrel.Mac `checkForUpdates` → `pruneUpdateDirectories` deletes the already-staged bundle. Quit during re-stage = ShipIt "Failed to copy bundle … no such file" → old version relaunches. Also electron-updater's `update-downloaded` fires before Squirrel staged, so "Install & Restart" waited silently.
- **Fix (`src/main.js`):** staged-lock (`runUpdateCheck` refuses once ready; in-flight guard w/ 30-min expiry; manual check re-offers restart), mac announces ready only after native `update-downloaded`, `installUpdate()` = isQuitting + 10s `app.exit` watchdog, persistent `updater.log` in `app.getPath("logs")`. Manual check feedback moved to events (downloading / up to date / failed / ready).
- **Verified:** fake-updater harness (electron-updater stubbed via `require.cache` EventEmitter, native `autoUpdater` events emitted by hand, dialogs/login-items/app.exit stubbed, isolated userData) over the REAL `src/main.js` — 11/11: launch check once, re-check after error, not announced pre-stage, in-flight manual skipped, announced once after native stage, post-stage manual = 0 checks, double install = 1 quitAndInstall, watchdog exit at 10s, log lines present.
- ⚠️ 1.2.1→1.2.2 still rides the OLD updater (may need a retry or manual dmg). The real proof is 1.2.2→1.2.3: read `~/Library/Logs/<App>/updater.log`.
- **Published 2026-09-26 19:37Z** (Keno approved; 8/8 assets, `latest.yml` + `latest-mac.yml` = 1.2.2, marked Latest).
